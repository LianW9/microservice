...
- Set the default compute zone to us-east1-b.
...
- Lists all clusters in the project, which is gke-cluster.
- ![3.5](images/i.png)

6. gcloud container clusters get-credentials gke-cluster \
  --zone us-east1-b
- Retrieves credentials so that kubectl can enable secure communication with the cluster by retrieving the cluster endpoint, download authentication credentials, and update the local kubeconfig file.
- ![3.6](images/j.png)

7. kubectl get nodes
- Verify that the cluster is connected and accessible, which is shown by three worker nodes in the Ready state.
- ![3.7](images/k.png)
...
- Displayed the repository to confirm the output and region.
...
- Automatically set in every Cloud session.
- ![4.5](images/p.png)
...
- Build and push the Products image by using the current directory as the build context for; uploading the source code and DockerFile, automatically Build the Docker image, and push the image to artifact Registry.
...
- Verify that the file was created as a Service, named product-service, with ClusterIp for the IP address, and the ports match (8082).
- ![6.5](images/c1.png)
...
- The Kubernetes automatically restores the wanted replica count.
- ![6.10](images/h1.png)
...
- Displays the Service selector for product-service, defines which Pods recieve from products-service.
- ![6.14](images/l1.png)

15. kubectl describe deployment products | grep Selector
- Shows the Deployment selector, defines which Pods belong to this Deployment's ReplicaSet.
- ![6.15](images/m1.png)
...
- This retrieves the vintage record player using its product ID.
- ![6.23](images/t1.png)

## Task 7
1. cd ~/gke-microservices-manifests
- Start in the gke-microservices-manifests directory.
- ![7.1](images/u1.png)
...
- Retrieves a single order showing that the Deployment was successful, the Service correctly selected the Pods, The CoreDNS resolved the Service name from inside the cluster, the Service name can be used instead of Pod IPs, and the port mapping in Service is correct.
- ![7.11](images/e2.png)
...
## Reflection
1. Deployment defines the wanted state in terms of image, replicas, and labels and is in control of the updates and rollbacks. A ReplicaSet is created by the Deployment and ensures that the correct number of Pods are running. While a Pod is the smallest unit of runtime and is ephemeral because of how easily replaceable they are. This lab assignment showed that deleting a Pod caused a new one to be created automatically with a different name and IP address. 

2. Clients use the Service name instead of Pod IPs because the Service name provides the opportunity for a stable ClusterIP and DNS name, while Pod IPs change when recreated. Clients would break if they used Pod IPs directly without the Service name. The Service uses selector to find matching Pods such as app=orders and app=products. Without Services there would be no reliable way of discovery, causing for manual tracking of Pod IPs without load balancing.
